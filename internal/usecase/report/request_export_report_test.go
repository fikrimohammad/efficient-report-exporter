...
func TestRequestExportReport_CreatesNewJob(t *testing.T) {
	...
	var publishedMsg mqmodel.ExportReportProcessMessage
	mq.EXPECT().
		PublishExportReportProcessMsg(gomock.Any(), gomock.Any()).
		DoAndReturn(func(_ context.Context, msg mqmodel.ExportReportProcessMessage) error {
			publishedMsg = msg
			return nil
		}).
		AnyTimes()
	...
	if publishedMsg.JobID != "42" {
		t.Fatalf("expected published JobID 42, got %s", publishedMsg.JobID)
	}
}
...
func TestRequestExportReport_LockAlreadyHeld(t *testing.T) {
	...
	mu := &useCase{
		...
		mqRepository:    mock.NewMockMQ(ctrl),
		redisRepository: redis,
		s3Repository:    mock.NewMockS3(ctrl),
		dynamicConfig:   dl,
	}
