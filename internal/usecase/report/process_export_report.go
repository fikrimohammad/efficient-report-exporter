...
func (e *reportExporter) runExportReportPipeline(ctx context.Context, shopID int64, startTime, endTime time.Time, jobID int64) error {
	...
	if total <= int64(maxSingleFileRows) {
		reportFileName, err = e.runSinglePipeline(mainPipeline, shopID, startTime, endTime, jobID)
	} else {
		reportFileName, err = e.runBatchedPipeline(mainPipeline, shopID, startTime, endTime, jobID)
	}
	if err != nil {
		return err
	}
	...
	if pubErr := e.mqRepository.PublishExportReportDoneMsg(ctx, mqmodel.ExportReportDoneMessage{
...
func (e *reportExporter) runSinglePipeline(mainPipeline *errgroup.Group, shopID int64, startTime, endTime time.Time, jobID int64) (string, error) {
	reports, fetchErr := e.asyncFetchReports(mainPipeline, shopID, startTime, endTime)
	if fetchErr != nil {
		return "", fetchErr
	}

	lines, lineErr := e.asyncBuildReportLine(mainPipeline, reports)
	if lineErr != nil {
		return "", lineErr
	}

	csv, csvErr := e.asyncBuildReportCSVFile(mainPipeline, lines)
	if csvErr != nil {
		return "", csvErr
	}
...
func (e *reportExporter) runBatchedPipeline(mainPipeline *errgroup.Group, shopID int64, startTime, endTime time.Time, jobID int64) (string, error) {
	batchFiles, batchErr := e.asyncBuildReportBatchFiles(mainPipeline, shopID, startTime, endTime)
	if batchErr != nil {
		return "", batchErr
	}
