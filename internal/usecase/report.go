...
func (p RequestExportReportParams) Validate() error {
	if p.ShopID == 0 {
		return errs.New(constant.InvalidArgument, "shop_id is required")
	}

	if p.StartTime.IsZero() {
		return errs.New(constant.InvalidArgument, "start_time is required")
	}

	if p.EndTime.IsZero() {
		return errs.New(constant.InvalidArgument, "end_time is required")
	}

	if p.StartTime.After(p.EndTime) {
		return errs.New(constant.InvalidArgument, "start_time is after end_time")
	}

	if p.EndTime.Sub(p.StartTime) > constant.MaxExportTimeRange {
		return errs.New(constant.InvalidArgument, "time range must not exceed 90 days")
	}
